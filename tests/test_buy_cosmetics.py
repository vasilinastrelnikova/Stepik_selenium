...
def test_buy_product():
    ...
    ffh.shampoo_to_cart()
    ffh.goto_description_broccoli_balm()
    bbp = Broccolibalm_page(driver)
    bbp.put_balm()
    bbp.goto_cart()
    cp = Cart_page(driver)
    price_shampoo_1 = cp.take_price(cp.get_shampoo_price())
    cp.plus_shampoo()
    time.sleep(2)
    cp.minus_shampoo()
    time.sleep(2)
    cp.plus_shampoo()
    time.sleep(3)
    price_shampoo_2 = cp.take_price(cp.get_shampoo_price())
    assert price_shampoo_1 < price_shampoo_2, "Не сработали кнопки +/- на продукте"

    cp.input_quantity_balm(3)
    time.sleep(2)
    cp.delete_mask()
    try:
        cp.delete_mask()
